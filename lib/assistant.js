...
class Assistant {
    constructor () {
        ...
        this.backend = null // Pluggable AI backend (DeepSeek direct or legacy FlowFuse)
    ...
    async init (RED, options = {}) {
        ...
        // Backend selection remains string-based so the handlers stay transport-
        // agnostic, but the old subprocess-migration scaffolding has now been removed.
        // The active path is direct DeepSeek, with the legacy FlowFuse HTTP
        // backend still available for compatibility.
     ...
     *
     * We keep backend selection here so the HTTP handlers can stay oblivious to
     * transport details. That lets us replace pi with direct provider calls while
     * preserving the same browser-facing REST contracts.
     *
     * @param {*} RED
     */
    _setupBackend (RED) {
        ...
        if (backendType === 'flowfuse') {
            ...
            // Direct DeepSeek transport. This avoids the extra process / agent-turn
            // overhead of the old subprocess backend, which matters most for
            // latency-sensitive FIM.
            // Ref: https://api-docs.deepseek.com
            const { DeepSeekBackend } = require('./ai/backends/deepseek.js')
            this.backend = new DeepSeekBackend({
                got: this.got,
                apiKey: this.options.apiKey || process.env.DEEPSEEK_API_KEY,
    ...
    async completeInitialization (clientSettings) {
        ...
        try {
            ...
            // and optional LLM integration for richer multi-step predictions.
            ...
            if (!completionsSupported) {
                ...
                // LLM integration for multi-step predictions fires on-demand via the
                // configured backend when needed.
    ...
    initAdminEndpoints (RED) {
        ...
        RED.httpAdmin.post('/nr-assistant/mcp/prompts/:promptId', RED.auth.needsPermission('write'), async function (req, res) {
            try {
                ...
                assistant.RED?.log?.error(`Unhandled error in handlePostPromptRequest: ${msg}`, err?.stack)
                if (!res.headersSent) {
                    res.status(500).json({ status: 'error', message: 'Internal server error' })
                }
            }
