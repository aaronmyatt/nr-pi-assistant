...
class PiRpcBackend extends AiBackend {
     ...
     */
    // ── FIM concurrency guard: only one FIM request at a time ──
    // Typing fires many FIM requests; concurrent ones race for agent_end.
    // We serialise them by tracking the in-flight request.
    _fimInFlight = false

    async run (req) {
        const { feature, prompt, context, transactionId, onDelta } = req

        // Reject concurrent FIM — the latest keystroke wins, old requests are stale
        if (feature === 'fim') {
        ...
        try {
            // FIM gets a short timeout (10s) — it's best-effort and concurrent
            // requests from rapid typing compete for agent_end events.
            // All other features use the configured timeout.
            const timeout = feature === 'fim' ? 10000 : this._timeout
            ...
            if (Array.isArray(result) && result.length > 0) {
                // Log unique event types once for debugging
                const types = [...new Set(result.map(e => e?.type).filter(Boolean))]
                this._log('debug', `pi-rpc response: ${result.length} events, types: ${types.join(', ')}`)
            }
